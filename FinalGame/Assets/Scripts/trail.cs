...
public class trail : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        points = new List<Vector3>() { transform.position + offsetVector }; //indices 1 - end are solidified points, index 0 is always transform.position
    ...
    void Update()
    {
        ...
        //add new point
        if (points.Count < 2 || Vector3.Distance(transform.position + offsetVector, points[1]) > minimumVertexDistance)
        {
            //if we have no solidified points, or we've moved enough for a new point
            AddPoint(transform.position + offsetVector);
        }

        //update index 0;
        points[0] = transform.position + offsetVector;
